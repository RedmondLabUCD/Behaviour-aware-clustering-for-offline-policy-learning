...
class PUBuffer():
    def __init__(self,
        ...
        self.pos_seed_dataset = pos_seed_dataset
        self.pu_seed_set_neg = pu_seed_set_neg
        if self.pu_seed_set_neg != None:
            self.use_neg_set = True
        else:
            self.use_neg_set = False
    ...
    def set_seed_positive(self):
        ...
        for i in range(self.args.models_in_ensemble):
            dataset = TorchDatasetHandler(
                self.sub_seed_positive(), self.raw_dataset, self.args, transform=None, use_neg_set=self.use_neg_set, neg_set=self.pu_seed_set_neg)
    ...
    def update_pos(self):
        ...
        for idx, lab in enumerate(self.relabel):
            if lab:
                ...
                if self.use_neg_set:
                    neg_obs.append(self.raw_dataset['observations'][idx])
                    neg_acts.append(self.raw_dataset['actions'][idx])
        self.neg_set['observations'] = np.array(neg_obs)
        self.neg_set['actions'] = np.array(neg_acts)
        self._init_torch_loaders()
        
    def init_torch_loader_to_train_policy(self):
        train_set = {'observations': [],'actions':[], 'terminals':[], 'rewards':[]}
        for idx, lab in enumerate(self.relabel):
            if lab:
                train_set['observations'].append(self.raw_dataset['observations'][idx])
                train_set['actions'].append(self.raw_dataset['actions'][idx])
                train_set['terminals'].append(self.raw_dataset['terminals'][idx])
        ...
        train_set['terminals'] = np.array(train_set['terminals'])
    ...
    def _init_torch_loaders(self):
        self.torch_loader = []
        if self.use_neg_set:
            print(f"Using the neg set with the size of {self.neg_set['actions'].shape[0]}")
            use_neg = True
            ...
            neg_set = None
        for i in range(self.args.models_in_ensemble):
            dataset = TorchDatasetHandler(self.buffers[i], self.raw_dataset, 
                                          self.args, transform=None,
                                          use_neg_set=use_neg, neg_set=neg_set)
            self.torch_loader.append(DataLoader(
                dataset, batch_size=self.args.batch_size, shuffle=True))
            
    def _get_pu_traj_labels(self, step_labels):
        traj_labels = []
        temp_traj_labels = []
        for idx, step_label in enumerate(step_labels):
            temp_traj_labels.append(step_label)
            if self.raw_dataset['terminals'][idx]:
                label = round(np.array(temp_traj_labels).mean())
                temp_traj_labels = []
    ...
    def classifier_validate(self, classifiers, log_path=None):
        with torch.no_grad():
            ...
            self.relabel_num = 0
            self.relabelled_idx = []
            for idx, timeout in enumerate(self.raw_dataset['terminals']):
                ...
                if timeout:
                    ...
                    temp_obs = []
                    temp_actions = []
            if self.args.fix_init_th_conf:
                if self.iter_num <= self.args.fix_init_th_iters - 1: #or self.iter_num == 4 or self.iter_num == 5:
                    adap_th = self.args.fix_init_th_conf_value
                else:
        ...
        return len(self.relabelled_idx), count, probs, self._get_pu_traj_labels(self.relabel), self.relabel_num, self.raw_len
...
class TorchDatasetHandler(Dataset):
    def __init__(self, filtered_dataset, raw_dataset, args=None, transform=None, use_neg_set = False, neg_set=None):
        self.args = args
    ...
    def __getitem__(self, index):
        ...
        if np.random.uniform(0, 1) <= 0.5:
            ...
            if self.use_neg_set:
                k = random.randint(0, self.neg_set['actions'].shape[0]-1)
                observation = self.neg_set['observations'][k]
                action = self.neg_set['actions'][k]
            else:
